...
/**
 * Taken from the Mediafragments library;
 * adapted to be able to register custom parsers
 */
...
var MediaFragments = (function(window) {
  ...
  return {
        registerDimensionParser: function(name, parserFun) {
            dimensions[name] = parserFun;
        },
    ...
    parseMediaFragmentsUri: function(opt_uri) {
      ...
      var hash = indexOfHash !== -1? uri.substring(indexOfHash + 1) : '';
            var queryValues = MediaFragments.splitKeyValuePairs(query);
            var hashValues = MediaFragments.splitKeyValuePairs(hash);
      return {
        query: queryValues,
        hash: hashValues,
        toString: function() {
         ...
         */
        splitKeyValuePairs: function(octetString) {
            var keyValues = {};
